...
#	Ver 2.0  2018/01/11 F.Kanehori	‘S‘Ì‚ÌŒ©’¼‚µ.
# ======================================================================
import sys
import os
...
class VisualStudio:
	...
	def build(self, platform, config):
		...
		if self.vs_path is None:
			...
			self.error = 'devenv not found: ' + msg
			return status
		...
		status = self.__build()
		if status != 0:
			self.error = 'build failed (%d)' % status
		#
		return status
	...
	def __get_vsinfo(self, toolset):
		...
		#
		if pts is None:
			self.error = 'invalid platform toolset: %s' % (toolset)
	...
	def __show_error(self, file):
		...
		try:
			...
			return None
		for line in f:
			if 'error' in line or 'Eorror' in line:
				print(line.strip())
		f.close()
...
if __name__ == '__main__':
	...
	if options.directory is None:
		...
		# Dailybuild control system is unavailable for this test.
