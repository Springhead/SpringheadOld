...
#  Windows (Visual Studio) の場合には次の処理も必要となる
#	Solution file に記されている参照プロジェクトの GUID を、リンクを張った
#	先の project file (Spr 側) の GUID に書き換える。
#
if is_windows():
	#  Spr 側にリンクした後の GUID を調べる
	#
	lnk_guids = collect_guid(app_blddir, projs)

	#  ソリューションファイルに現れる GUID をリンク先のものに置き換える
	#	
