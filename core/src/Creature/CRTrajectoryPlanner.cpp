...
namespace Spr {
	...
	void CRTrajectoryPlanner::HingeJoint::SetPullbackTarget(int k, int n) {
		if (k >= 0 && k <= iterate) {
			hinge->SetPullbackTarget(angle[k][n]);
		}
	}

	void CRTrajectoryPlanner::HingeJoint::SetPullbackTargetFromInitial() {
	...
	void CRTrajectoryPlanner::BallJoint::SetPullbackTarget(int k, int n) {
		if (k >= 0 && k <= iterate) {
			ball->SetPullbackTarget(ori[k][n]);
		}
	}

	void CRTrajectoryPlanner::BallJoint::SetPullbackTargetFromInitial() {
	...
	void CRTrajectoryPlanner::Joints::SetPullbackTarget(int k, int n) {
		for (size_t i = 0; i < joints.size(); i++) {
			joints[i]->SetPullbackTarget(k, n);
		}
	}

	void CRTrajectoryPlanner::Joints::SetPullbackTargetFromInitial() {
		for (size_t i = 0; i < joints.size(); i++) {
	...
	void CRTrajectoryPlanner::OutputTrajectory(std::string filename) {
		...
		int l = (int)torqueChangeRecord.size();
		for (int i = 0; i < l; i++) {
