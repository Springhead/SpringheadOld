...
%	Last update: 2019/01/04 F.Kanehori
%\newpage
...
\begin{Method}{\plAryR{buff} = gather_lines(\plHshR{modules}, \plVar{module})}
    ...
    \end{Brief}
\end{Method}
\begin{Method}{\plVar{result} = is_in_buff(\plAryR{buff}, \plVar{data})}
    \begin{Args}
	\Item[t]{\plAryR{buff}}{データの存在を確認する文字列の配列}
	\Item[t]{\plVar{data}}{データ文字列}
    \end{Args}
    \begin{Return}
	\Item[t]{\plVar{result}}{一致する要素があったら``真''、なかったら``偽''}
    \end{Return}
    \begin{Brief}
	与えられたデータ文字列が、指定された文字列配列のいずれかの要素に
