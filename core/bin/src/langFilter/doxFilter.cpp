...
void WriteSec(const char* b, const char* e){
	...
	out << cmdStr << " " << idStr << " " << chapStr << secCount[0] << "　";
}
void WriteSubSec(const char* b, const char* e){
	...
	out << cmdStr << " " << idStr << " " << chapStr << secCount[0] << "." << secCount[1] << "　";
}
void WriteSubSubSec(const char* b, const char* e){
	...
	out << cmdStr << " " << idStr << " " << chapStr << secCount[0] << "." << secCount[1] << "." << secCount[2] << "　";
}
void WriteParagraph(const char* b, const char* e){
	if (curLang.length() && curLang.compare(lang)!=0) return;
	secCount[3]++;
	secs.push_back(SecInfo(3, idStr));
	out << cmdStr << " " << idStr << " " << chapStr << secCount[0] << "." << secCount[1] << "." << secCount[2] << "." << secCount[3] << "　";
}
...
int _cdecl main(int argc, const char* argv[]){
	...
	char cmd[256] = "nkf -Sw ";
	strcat(cmd, argv[2]);
	strcat(cmd, " >langFilter.tmp");
	system(cmd);
	ifstream file("langFilter.tmp");
	...
	for(unsigned i=0; i<contents.size(); ++i){
		...
		curPos = cpos+1;
		cout << "\\htmlonly\n";
		cout << "<h2><a class=\"anchor\">◇ Contents ◇</a></h2>\n";
		cout << "\\endhtmlonly\n";
