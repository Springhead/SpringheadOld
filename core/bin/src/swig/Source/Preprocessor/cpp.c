...
/*
static int SJIS_FIRST(int c){
	if ((0x81<=c && c <= 0x9F) || (0xE0<=c && c <=0xFF)) return 1;
	return 0;
}
*/
static int UTF_FIRST(int c){	//	あと何バイトか返す。
	if ((c & 0xF0) == 0xF0) return 3;
	if ((c & 0xF0) == 0xE0) return 2;
	if ((c & 0xF0) == 0xC0) return 1;
	return 0;
}
...
/*	  if (c > 127 && utf==0){
		int n = UTF_FIRST(c);
		printf("UTF: (%d:%c  %d %d) ", c,c, n, state);
	  }
 */
    switch (state) {
